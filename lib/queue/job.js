...
Job.prototype.save = function( fn, options = {} ) {
  var client = this.client
    , fn     = fn || noop
  ...
  if (id && typeof id !== 'number') {
    throw new TypeError(`Job ID must be of type number`);
  }

  function getJobId( done ) {
    if (id) {
      done(null, id)
    }

    // incr id if none is given
    client.incr(client.getKey('ids'), done)
  }

  // fetch the ID to use
  getJobId(function( err, id ) {
    if( err ) return fn(err);
